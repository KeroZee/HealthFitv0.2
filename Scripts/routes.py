...
from Scripts import ScheduleTodolist, HealthTracker
...
def schedule():
    ...
    return render_template('schedule.html', image_file=image_file, today_quote=today_quote, today_quote_author=today_quote_author)
...
def Healthtracker():
    image_file = url_for('static', filename='profile_pics/' + current_user.image_file)
    heartrate = 100 #int(request.form.get("record"))
    c1 = HealthTracker.Condition
    r1 = HealthTracker.Recommend
    #shelving = shelve.open('store_healthtracker')
    #health = ""
    healthtracker_condition = c1
    healthtracker_recommend = r1
    condition1 = "Poor"
    recommend1 = "Adjusting or discontinuing your medical regimens, Using a pacemaker. You may not need treatment if you don't have symptoms."
    condition2 = "Good"
    recommend2 = "Having a mixture of sedentary, vigorous and moderate intensity activities/exercises to ensure that your heart rate stays in this condition. You will not need treatment unless your heart condition changes."
    condition3 = "Critical"
    recommend3 = "Carotid sinus massage, Pressing gently on the eyeballs with eyes closed under the supervision of a healthcare physician, Valsalva manoeuvre, Using the dive reflex, Sedation, Cutting down on coffee or caffeinated substances, Cutting down on alcohol, Quitting tobacco use, Getting more rest."
    condition4 = "Hazardous"
    recommend4 = "Carotid sinus massage, Taking medication including digoxin, verapamil and beta blockers, Electric shock treatment, Catheter ablation, Valsalva manoeuvre, Dipping your face into cold water during an attack. In many cases, the symptoms stop quickly and no treatment is needed."
    if int(heartrate) < 60:
        condition_1 = healthtracker_condition(condition1)
        recommend_1 = healthtracker_recommend(condition_1, recommend1)
        current_condition = condition_1
        current_recommend = recommend_1

    elif int(heartrate) >= 60 or int(heartrate) <= 100:
        condition_2 = healthtracker_condition(condition2)
        recommend_2 = healthtracker_recommend(condition_2, recommend2)
        current_condition = condition_2
        current_recommend = recommend_2

    elif int(heartrate) > 100 or int(heartrate) <= 140:
        condition_3 = healthtracker_condition(condition3)
        recommend_3 = healthtracker_recommend(condition_3, recommend3)
        current_condition = condition_3
        current_recommend = recommend_3

    elif int(heartrate) > 140:
        condition_4 = healthtracker_condition(condition4)
        recommend_4 = healthtracker_recommend(condition_4, recommend4)
        current_condition = condition_4
        current_recommend = recommend_4

    #while health == "":
        #if int(heartrate) < 60:
            #health = shelving['heart0']

        #elif 60 <= int(heartrate) <= 100:
            #health = shelving['heart1']

        #elif 100 < int(heartrate) <= 140:
            #health = shelving['heart2']

        #elif int(heartrate) > 140:
            #health = shelving['heart3']

    #shelving.close()
    return render_template('HealthTracker.html',image_file=image_file, heartrate=heartrate, current_condition=current_condition, current_recommend=current_recommend)
...
=======
    ...
    if form.meal.data == 'breakfast':
        ...
        searches = ""
    db.session.commit()
>>>>>>> 7f27c91fdef44459501f87d8f41ec4185221f28d
